...
function assertSerializablePayload(question) {
  const serialized = JSON.stringify(question.payload);
...
function assertSimpleValue(value) {
  if (typeof value === "number") {
    assert.ok(Number.isInteger(value), `Expected simple integer value, got ${value}`);
    assert.ok(value >= 0 && value <= 100, `Expected age-appropriate value, got ${value}`);
  }
}

function wrongAnswerFor(question) {
  if (question.format === "trueFalse") {
    return question.expectedAnswer === true ? "false" : "true";
  }

  if (question.format === "multipleChoice") {
    return question.options.find((option) => !option.isCorrect).optionId;
  }

  if (typeof question.expectedAnswer === "number") {
    return question.expectedAnswer + 1;
  }
  ...
  for (const level of levels) {
    for (const format of supportedQuestionFormats) {
      const question = generateQuestion(level, {
        format,
        seed: `${level.levelId}:${format}:primary`
      });

      assert.ok(question.questionTemplateId);
      ...
      assert.equal(question.payload.correctAnswer, question.payload.correctAnswer);
      assertSerializablePayload(question);
      assertSimpleValue(question.payload.correctAnswer);
      assertSimpleValue(question.expectedAnswer);

      const correctResult = checkAnswer(question, question.expectedAnswer);
      assert.equal(correctResult.isCorrect, true);

      const wrongResult = checkAnswer(question, wrongAnswerFor(question));
...
test("multiple choice questions have four options and exactly one correct answer", () => {
  for (const level of levels) {
    ...
    assert.equal(
      question.options.find((option) => option.isCorrect).optionId,
...
test("answer checking accepts simple typed equivalents centrally", () => {
  const numericQuestion = generateQuestion(levels[3], {
    format: "solve",
    seed: "numeric-equivalent"
  });
  ...
  const trueFalseQuestion = generateQuestion(levels[0], {
    format: "trueFalse",
    seed: "true-false-equivalent"
  });
  const submitted = trueFalseQuestion.expectedAnswer ? "t" : "f";
  assert.equal(checkAnswer(trueFalseQuestion, submitted).isCorrect, true);
});
